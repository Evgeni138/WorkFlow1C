---
name: inspect
description: "Этот скилл MUST быть вызван для анализа и инспекции любого объекта 1С из XML-выгрузки: конфигурация (cf-info), объект метаданных (meta-info), управляемая форма (form-info), схема компоновки данных СКД (skd-info), макет табличного документа MXL (mxl-info), подсистема (subsystem-info), роль и права (role-info). SHOULD вызывать как подготовительный шаг перед любой модификацией объекта. Do NOT использовать для модификации — для изменений используй соответствующие edit/compile скиллы."
---
