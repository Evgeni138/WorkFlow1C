...
$replacements = @{
    ...
    # Gitea token (укажите свой перед использованием)
    'YOUR_GITEA_TOKEN_REAL' = 'YOUR_GITEA_TOKEN'
...
# Cursor files (legacy, if present - only for projects started from the original kit)
...
# opencode files
$files += Get-ChildItem ".opencode\skills\*.md" -Recurse -ErrorAction SilentlyContinue
$files += Get-ChildItem ".opencode\command\*.md" -Recurse -ErrorAction SilentlyContinue
$files += Get-ChildItem "opencode.jsonc" -ErrorAction SilentlyContinue
$files += Get-ChildItem "AGENTS.md" -ErrorAction SilentlyContinue
